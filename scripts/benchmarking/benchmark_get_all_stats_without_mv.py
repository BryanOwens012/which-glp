#!/usr/bin/env python3
"""
Benchmark: getAllStats WITHOUT using materialized view (direct table queries)

This script benchmarks the getAllStats query when executed directly on the base tables
...
def main():
    ...
    print("Benchmarking getAllStats WITHOUT materialized view (direct table queries)...\n")

    # Query that computes drug stats directly from base tables
    # This is the equivalent of get_drug_stats() but without the MV
    query = """
EXPLAIN ANALYZE
...
"""
    ...
    try:
        ...
        # Run EXPLAIN ANALYZE on getAllStats query (WITHOUT MV)
        execute_query(
            conn,
            query,
            "Benchmarking getAllStats WITHOUT Materialized View (Direct Table Queries)"
        )
