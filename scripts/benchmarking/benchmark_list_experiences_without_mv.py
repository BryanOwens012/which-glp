#!/usr/bin/env python3
"""
Benchmark: List experiences WITHOUT using materialized view (direct table queries)

This script benchmarks the experiences.list query when executed directly on the base tables
(extracted_features, reddit_posts, reddit_comments) WITHOUT using the materialized view.

This represents the non-optimized query path and should be slower.
"""
...
def main():
    ...
    print("Benchmarking list experiences WITHOUT materialized view (direct table queries)...\n")

    # Query that lists experiences directly from base tables
    # This reconstructs the MV logic on-the-fly with filters, sorting, and pagination
    query = """
EXPLAIN ANALYZE
...
"""
    ...
    try:
        ...
        # Run EXPLAIN ANALYZE on list experiences query (WITHOUT MV)
        execute_query(
            conn,
            query,
            "Benchmarking List Experiences WITHOUT Materialized View (Direct Table Queries)"
        )
