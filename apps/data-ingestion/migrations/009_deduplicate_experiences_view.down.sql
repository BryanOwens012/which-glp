-- Rollback: Revert to previous view without deduplication
-- This recreates the view from migration 008

-- Drop the deduplicated view
DROP MATERIALIZED VIEW IF EXISTS mv_experiences_denormalized CASCADE;

-- Recreate the previous version (without DISTINCT ON)
...
COMMENT ON MATERIALIZED VIEW mv_experiences_denormalized IS
  'Denormalized view of experiences with pre-calculated metrics including weight loss speed.
